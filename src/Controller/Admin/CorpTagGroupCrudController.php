...
/**
 * @extends AbstractCrudController<CorpTagGroup>
 */
#[AdminCrud(routePath: '/wechat-work/corp-tag-group', routeName: 'wechat_work_corp_tag_group')]
final class CorpTagGroupCrudController extends AbstractCrudController
{
    public static function getEntityFqcn(): string
    {
        return CorpTagGroup::class;
    }

    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ->setEntityLabelInSingular('企业标签分组')
            ->setEntityLabelInPlural('企业标签分组')
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        yield TextField::new('name', '分组名')
            ->setRequired(true)
            ->setHelp('标签分组的名称，最多120个字符')
        ;

        yield TextField::new('remoteId', '远程ID')
            ->hideOnForm()
            ->setHelp('企业微信系统中的分组ID')
        ;
        ...
        yield AssociationField::new('items', '包含标签')
            ->onlyOnDetail()
